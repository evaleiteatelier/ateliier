  ...
  async function carregarPedidos(filtro, destino, botaoAcao, novoStatus, termoPesquisa = "") {
  let query = supabase
    .from('pedidos')
    .select('*')
    .eq('status', filtro)
  ...
  // [LINHAS NOVAS]
  // Se houver um termo de pesquisa, adiciona o filtro 'ilike' (case-insensitive)
  if (termoPesquisa) {
    query = query.ilike('nome', `%${termoPesquisa}%`);
  }
...
  container.innerHTML = '';
  ...
  // 1. Carrega os pedidos iniciais (sem filtro)
  carregarPedidos('pendente', 'lista-espera', 'Concluir', 'concluido');

  // --- 👇 O CÓDIGO DE PESQUISA VEM AQUI 👇 ---
  const campoPesquisa = document.getElementById('pesquisa');
  if (campoPesquisa) {
    // Usamos 'input' para ser instantâneo
    campoPesquisa.addEventListener('input', () => {
      const termo = campoPesquisa.value; // Não precisa de toLowerCase()
          
      // 2. Chama a função carregarPedidos DE NOVO, mas com o filtro
      carregarPedidos('pendente', 'lista-espera', 'Concluir', 'concluido', termo);
    });
  }
  // --- 👆 FIM DA ALTERAÇÃO 👆 ---
}
